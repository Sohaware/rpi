...
HELPER_SHA256="f814045f2222ce136bb798f04c22fc7020ca6a27d5d6ade49e303fbe6bede210"
MANIFEST_SHA256="0b8c7ebfc70571db6a44ff8857c03472d6d8d02230e32948de6b72fe0a177cd5"
VERSION_STATUS_SHA256="03cfb9213c2263f0128373c58b9847832c5cc36e2ca89e7fd93d00de50e48796"
BASE="https://raw.githubusercontent.com/Sohaware/rpi/${TAG}"
echo "CodyNick core bootstrap ${VERSION} (TTS permissions repair)"
if [[ $EUID -ne 0 ]]; then
...
wget --timeout=60 --tries=3 -O "$temp/core-manifest.json" "$BASE/releases/core-0.6.1.json"
...
if json.loads(pathlib.Path(sys.argv[1]).read_text()).get('version') != '0.6.1':
    raise SystemExit('Incorrect release manifest')
PY
install -d -m 0755 /usr/local/lib/codynick/core-0.6.1
install -m 0644 "$temp/app_setup.py" /usr/local/lib/codynick/core-0.6.1/app_setup.py
install -m 0644 "$temp/core-manifest.json" /usr/local/lib/codynick/core-0.6.1/core-manifest.json
install -m 0644 "$temp/version_status.py" /usr/local/lib/codynick/core-0.6.1/version_status.py
install -m 0644 "$temp/vision_setup.py" /usr/local/lib/codynick/core-0.6.1/vision_setup.py
install -m 0644 "$temp/speech_setup.py" /usr/local/lib/codynick/core-0.6.1/speech_setup.py
install -m 0644 "$temp/ocr_setup.py" /usr/local/lib/codynick/core-0.6.1/ocr_setup.py
install -m 0644 "$temp/tts_setup.py" /usr/local/lib/codynick/core-0.6.1/tts_setup.py
...
systemd-run --unit=codynick-install --collect --property=Type=exec \
    /usr/bin/python3 -u /usr/local/lib/codynick/core-0.6.1/app_setup.py --worker
echo
...
echo 'Success ends with: CodyNick 0.6.1: READY'
