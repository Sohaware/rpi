...
HELPER_SHA256="4c04b53af40f8f7aaf282e8b7646ddefde03e279c21737d0cdb65a6f21eb3b99"
MANIFEST_SHA256="cb36d0154dd4ddda42e908822fba9b710c720b61ccf9727b2f08672721976154"
VERSION_STATUS_SHA256="2b9c51ae15fe090ef58caff2c48345718a4e5db97b78c43c7a3eb823c3e61e54"
BASE="https://raw.githubusercontent.com/Sohaware/rpi/${TAG}"
echo "CodyNick core bootstrap ${VERSION} (managed examples polish)"
if [[ $EUID -ne 0 ]]; then
...
wget --timeout=60 --tries=3 -O "$temp/core-manifest.json" "$BASE/releases/core-0.5.3.json"
...
if json.loads(pathlib.Path(sys.argv[1]).read_text()).get('version') != '0.5.3':
    raise SystemExit('Incorrect release manifest')
PY
install -d -m 0755 /usr/local/lib/codynick/core-0.5.3
install -m 0644 "$temp/app_setup.py" /usr/local/lib/codynick/core-0.5.3/app_setup.py
install -m 0644 "$temp/core-manifest.json" /usr/local/lib/codynick/core-0.5.3/core-manifest.json
install -m 0644 "$temp/version_status.py" /usr/local/lib/codynick/core-0.5.3/version_status.py
install -m 0644 "$temp/vision_setup.py" /usr/local/lib/codynick/core-0.5.3/vision_setup.py
install -m 0644 "$temp/speech_setup.py" /usr/local/lib/codynick/core-0.5.3/speech_setup.py
install -m 0644 "$temp/ocr_setup.py" /usr/local/lib/codynick/core-0.5.3/ocr_setup.py
...
systemd-run --unit=codynick-install --collect --property=Type=exec \
    /usr/bin/python3 -u /usr/local/lib/codynick/core-0.5.3/app_setup.py --worker
echo
...
echo 'Success ends with: CodyNick 0.5.3: READY'
