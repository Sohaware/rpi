...
HELPER_SHA256="534039db1a79e53966ef383697692d0321b0d2caf8eb392523d1042d8291e5de"
MANIFEST_SHA256="ee363926d8057385364b404733b8d2cdd4449cefc550c11de66c52e299e96649"
VERSION_STATUS_SHA256="53355174d06754184e578bdf00ecbc6d5b1128a25f6196b9a4f3338349804490"
...
wget --timeout=60 --tries=3 -O "$temp/core-manifest.json" "$BASE/releases/core-0.7.4.json"
...
if json.loads(pathlib.Path(sys.argv[1]).read_text()).get('version') != '0.7.4':
    raise SystemExit('Incorrect release manifest')
PY
install -d -m 0755 /usr/local/lib/codynick/core-0.7.4
install -m 0644 "$temp/app_setup.py" /usr/local/lib/codynick/core-0.7.4/app_setup.py
install -m 0644 "$temp/core-manifest.json" /usr/local/lib/codynick/core-0.7.4/core-manifest.json
install -m 0644 "$temp/version_status.py" /usr/local/lib/codynick/core-0.7.4/version_status.py
install -m 0644 "$temp/vision_setup.py" /usr/local/lib/codynick/core-0.7.4/vision_setup.py
install -m 0644 "$temp/speech_setup.py" /usr/local/lib/codynick/core-0.7.4/speech_setup.py
install -m 0644 "$temp/ocr_setup.py" /usr/local/lib/codynick/core-0.7.4/ocr_setup.py
install -m 0644 "$temp/tts_setup.py" /usr/local/lib/codynick/core-0.7.4/tts_setup.py
...
systemd-run --unit=codynick-install --collect --property=Type=exec \
    /usr/bin/python3 -u /usr/local/lib/codynick/core-0.7.4/app_setup.py --worker
echo
...
echo 'Success ends with: CodyNick 0.7.4: READY'
