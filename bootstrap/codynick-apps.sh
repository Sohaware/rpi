...
HELPER_SHA256="699114d92fbf71b2327d8c30abc96b2c90d1007820dfb90c4d002f151bf5e2aa"
MANIFEST_SHA256="cb0fb22b74d67669aac2813850b92a078afe6e75c5b0d5fd3401c144da339f0f"
VERSION_STATUS_SHA256="028661e21a54019139860d573467372bb0e36b30bfe5b8bd05c2cd466b7d4114"
BASE="https://raw.githubusercontent.com/Sohaware/rpi/${TAG}"
echo "CodyNick core bootstrap ${VERSION} (IoT and AI API reference)"
if [[ $EUID -ne 0 ]]; then
...
wget --timeout=60 --tries=3 -O "$temp/core-manifest.json" "$BASE/releases/core-0.7.1.json"
...
if json.loads(pathlib.Path(sys.argv[1]).read_text()).get('version') != '0.7.1':
    raise SystemExit('Incorrect release manifest')
PY
install -d -m 0755 /usr/local/lib/codynick/core-0.7.1
install -m 0644 "$temp/app_setup.py" /usr/local/lib/codynick/core-0.7.1/app_setup.py
install -m 0644 "$temp/core-manifest.json" /usr/local/lib/codynick/core-0.7.1/core-manifest.json
install -m 0644 "$temp/version_status.py" /usr/local/lib/codynick/core-0.7.1/version_status.py
install -m 0644 "$temp/vision_setup.py" /usr/local/lib/codynick/core-0.7.1/vision_setup.py
install -m 0644 "$temp/speech_setup.py" /usr/local/lib/codynick/core-0.7.1/speech_setup.py
install -m 0644 "$temp/ocr_setup.py" /usr/local/lib/codynick/core-0.7.1/ocr_setup.py
install -m 0644 "$temp/tts_setup.py" /usr/local/lib/codynick/core-0.7.1/tts_setup.py
...
systemd-run --unit=codynick-install --collect --property=Type=exec \
    /usr/bin/python3 -u /usr/local/lib/codynick/core-0.7.1/app_setup.py --worker
echo
...
echo 'Success ends with: CodyNick 0.7.1: READY'
