...
spec = importlib.util.spec_from_file_location(
    "speech_setup", Path(__file__).parents[1] / "installer/speech_setup.py"
)
...
class SpeechSetupTests(unittest.TestCase):
    def test_runtime_link_allowed_but_external_link_rejected(self):
        member = tarfile.TarInfo("home/client/.codynick-ai/envs/stt/bin/python")
    ...
    def test_unowned_and_special_paths_rejected(self):
        for name in (
            ...
            "home/client/.vosk/another-model/file",
        ):
            with self.assertRaises(ValueError):
                m.validate_member(tarfile.TarInfo(name))
        device = tarfile.TarInfo("home/client/.codynick-ai/envs/stt/device")
    ...
    def test_restore_is_repeatable_and_preserves_unmanaged_files(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            target = root / "home/client/.codynick-ai/envs/stt/module.py"
            archive = root / "speech.tar.gz"
            with tarfile.open(archive, "w:gz") as bundle:
                member = tarfile.TarInfo(
                    "home/client/.codynick-ai/envs/stt/module.py"
                )
                member.size = 5
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with tarfile.open(archive, "w") as bundle:
                link = tarfile.TarInfo(
                    "home/client/.codynick-ai/envs/stt/link"
                )
